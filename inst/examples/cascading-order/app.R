...
cfg <- table_config(
  ...
  columns = list(
    ...
    widget_col("category", "dropdown", "Category",
      width = 150,
      options = list(
    ...
    widget_col("instructions", "text", "Special Instructions",
      min_width = 200,
      options = list(
...
ui <- fluidPage(
  ...
  p(class = "text-muted",
    "Pick a ", tags$b("Category"), " to see matching ",
    tags$b("Items"), " appear in the next column (cascading choices).",
    " Add rows with ", tags$b("Add Row"), ", remove with the ",
    tags$b("\u00d7"), " button, or ", tags$b("Reset"),
...
server <- function(input, output, session) {
  ...
  output$summary_tbl <- renderReactable({
    ...
    df$item_label <- vapply(seq_len(nrow(df)), function(i) {
      cat <- df$category[i]
      itm <- df$item[i]
      if (is.na(cat) || is.na(itm)) return(NA_character_)
      items <- menu[[cat]]
      if (is.null(items)) return(itm)
