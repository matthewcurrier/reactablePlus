...
#'     \item{`"appendable"`}{Appendable fruit order — user-managed
#'       rows with add/delete buttons, min_rows/max_rows constraints,
#'       dropdown + numeric + checkbox + text columns.}
#'   }
...
runExample <- function(example = NULL, port = NULL) {
  ...
  if (is.null(example)) {
    ...
    descriptions <- c(
      ...
      appendable = "Appendable fruit order — user-managed rows, add/delete"
    )
