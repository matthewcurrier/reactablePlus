...
The lower-level module accepts plain lists describing rows and columns. You
control every column type (dropdown, numeric, checkbox, toggle, date, text)
and can wire gating, selection, and reset behavior.
...
Use `validate_col_spec()` and `validate_row_spec()` to catch configuration
errors before the app launches.
