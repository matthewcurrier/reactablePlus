...
dashed-border trigger button. When filled, renders a card showing the
selected result's details and action links (change, fill down, clear).

`schoolPickerInput()` is a deprecated alias for `searchPickerInput()`.
...
- ...:
  ...
  Arguments passed to `searchPickerInput()`.
...
Trigger and popover labels are configurable so the widget can be used
for any server-side typeahead scenario (school search, provider lookup,
facility search, etc.).
