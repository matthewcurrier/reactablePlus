...
describe("validate_col_spec()", {
  ...
  describe("given a dropdown col_spec entry", {
    ...
    it("throws an error when choices is a plain character vector, not a list", {
      spec             <- valid_col_spec()
  ...
  describe("given a col_spec containing all six supported types", {
    it("returns invisibly without error", {
      expect_no_error(validate_col_spec(valid_full_col_spec(), valid_row_spec()))
    })
  })
  ...
  describe("given a valid value-only gate", {
    it("accepts the value_gate_col_spec without error", {
      expect_no_error(validate_col_spec(value_gate_col_spec(), valid_row_spec()))
    })
  })

  describe("given a valid selection-only gate", {
    it("accepts the selection_gate_col_spec without error", {
      expect_no_error(validate_col_spec(selection_gate_col_spec(), selectable_row_spec()))
    })
  })

  describe("given a valid chained gate", {
    it("accepts the chained_gate_col_spec without error", {
      expect_no_error(validate_col_spec(chained_gate_col_spec(), selectable_row_spec()))
    })
  })
  ...
  describe("given a gate value condition missing col_name", {
    it("throws an error mentioning col_name", {
      ...
      expect_error(validate_col_spec(spec, valid_row_spec()), regexp = "col_name")
    })
  })
  ...
  describe("given a gate value condition whose col_name does not exist in the spec", {
    it("throws an error naming the missing col_name", {
      ...
      expect_error(validate_col_spec(spec, valid_row_spec()), regexp = "nonexistent_col")
    })
  })
    ...
    it("throws an error — controller must be declared before dependent", {
      spec <- list(
        list(
          ...
          type     = "numeric",
          min      = -24L,
          gate     = list(list(type = "value", col_name = "progress", values = c("early")))
        ),
        list(
          col_name = "progress",
          label    = "Progress",
      ...
      expect_error(validate_col_spec(spec, valid_row_spec()), regexp = "progress")
    })
  })
    ...
    it("throws an error mentioning dropdown", {
      spec <- list(
        ...
        list(
          ...
          type     = "numeric",
          min      = -24L,
          gate     = list(list(type = "value", col_name = "suspensions", values = c("1")))
        )
      )
      expect_error(validate_col_spec(spec, valid_row_spec()), regexp = "dropdown")
    })
  })
  ...
  describe("given a gate value condition with values not in controller choices", {
    it("throws an error naming the invalid value", {
      ...
      expect_error(validate_col_spec(spec, valid_row_spec()), regexp = "nonexistent_value")
    })
  })
})
